        ...
        case TA_TXPOLL_WAIT_SEND :
            {
				...
				dwt_setrxtimeout((uint16)inst->fwtoTime_sy*4);  //units are us - wait for 7ms after RX on (but as using delayed RX this timeout should happen at response time + 7ms)
                        ...
                        switch(fcode)
                        {
                            case RTLS_DEMO_MSG_RNG_INIT:
                            {
                                ...
                                if(inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
                                {
									...
									//memcpy(&inst->msg.sourceAddr[0], &srcAddr[0], ADDR_BYTE_SIZE_L); //2014.11.26 PJB
#else
