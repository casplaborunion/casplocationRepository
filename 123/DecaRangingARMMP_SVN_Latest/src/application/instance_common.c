...
//NOTE: Do not go back to "discovery" mode -  needs a boards reset to go back to "discovery" mode once is starts ranging
		...
		if(inst->mode == TAG)
		{
			...
			if(inst->previousState !=  TA_TX_WAIT_CONF) //TA_TX_WAIT_CONF TA_TXFINAL_WAIT_SEND
			{
//				if(inst->anchorListIndex > 1)
					inst->anchorListIndex--;
//				else
//					inst->anchorListIndex = 0;
			}
//			else if(inst->previousState == TA_RX_WAIT_DATA)
//				inst->anchorListIndex -= 1;
...
int instance_run(void)
{
    ...
    {
        if(instance_data[instance].mode == TAG) //Tag (is eithe0r in RX or sleeping)
        {
