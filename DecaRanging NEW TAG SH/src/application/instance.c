...
#else
	inst->msg.frameCtrl[1] = 0x8 /*dest short address (16bits)*/
	| 0x80 /*src short address (16bits)*/;
#endif
...
int destaddress(instance_data_t *inst) {
	...
	while (getnext) {
		if ((0x1 << inst->anchorListIndex) & inst->payload.anchorPollMask) {
			memcpy(&inst->msg.destAddr[0],
					&inst->payload.anchorAddressList[inst->anchorListIndex],
					ADDR_BYTE_SIZE_L);
			getnext = 0;
		}
...
int masterreplyaddress(instance_data_t *inst) {
	int tempmasterListIndex = 3;
	memcpy(&inst->msg.destAddr[0],
			&inst->payload.anchorAddressList[inst->masterListIndex],
			ADDR_BYTE_SIZE_L);
	//if we got this far means that we are just about to poll the first anchor in the list
	inst->instToSleep = 0; //we'll sleep after this reply

	return 0;
}	//2015.03.05 JSH masterreplyaddress
#endif
...
// -------------------------------------------------------------------------------------------------------------------
//
void setupmacframedata(instance_data_t *inst, int len, int framectrllen,
		int fcode, int ack) {
...
int testapprun_s(instance_data_t *inst, int message) {
	...
	case TA_INIT:
		...
		case TAG: {
#if (DR_DISCOVERY == 0)
			uint8 eui64[8];
			memcpy(eui64, &inst->payload.tagAddress, sizeof(uint64));
			...
			inst->testAppState = TA_CALL_WAIT;
...
//#endif
		...
		case ANCHOR: {
...
#if (USING_64BIT_ADDR==0)
			{
				...
				memcpy(&inst->msg.sourceAddr[0], inst->eui64, ADDR_BYTE_SIZE_S);
				//set source address into the message structure
				memcpy(&inst->rng_initmsg.sourceAddr[0], inst->eui64,
						ADDR_BYTE_SIZE_S);
			}
#else
#if (DR_DISCOVERY == 0)
...
//#endif
	...
	case TA_TXE_WAIT: //either go to sleep or proceed to TX a message
		...
		memcpy(&dataseq[0], (const uint8 *) "  ARRVIED     LL", 16);
		...
		if (((inst->nextState == TA_TXPOLL_WAIT_SEND)
			...
			inst->testAppState = TA_INIT;				//2015.01.26 jsh
			inst->nextState = TA_CALL_WAIT; 			//2015.01.26 jsh
			//inst->testAppState = TA_CALL_WAIT;		//2015.01.26 jsh
			//inst->nextState = TA_CALL_WAIT_DATA; 		//2015.01.26 jsh
			inst->instToSleep = 0;						//2015.01.26 jsh
	...
	case TA_TXRANGINGINIT_WAIT_SEND: {
		...
		inst->rng_initmsg.messageData[RES_T1] =
				((int) (inst->fixedReplyDelay_ms)) & 0xFF;
		inst->rng_initmsg.messageData[RES_T2] =
				(((int) (inst->fixedReplyDelay_ms)) >> 8) & 0xFF; //
	...
	case TA_CALL_WAIT: {
		...
		writetoLCD(40, 1, dataseq); //send some data
		memcpy(&dataseq[0], (const uint8 *) "    WAITING     ", 16);
		writetoLCD(16, 1, dataseq); //send some data
		inst->testAppState = TA_CALL_WAIT_DATA;   // let this state handle it
	...
	case TA_CALL_WAIT_DATA:                                      // Wait RX data
		...
		memcpy(&dataseq[0], (const uint8 *) "   CALLING      ", 16);
		writetoLCD(40, 1, dataseq);  //send some data
		memcpy(&dataseq[0], (const uint8 *) "  WAITING_DATA  ", 16);
		writetoLCD(16, 1, dataseq);  //send some data
        ...
        case SIG_RX_ACK :
        {
            ...
            {
                inst->testAppState = TA_CALL_WAIT ;              // wait for next frame
            }
            inst->done = INST_NOT_DONE_YET;
        }
        break;
		...
		case DWT_SIG_RX_OKAY: {
			...
			if ((inst->ackexpected) && (inst->ackTO)) //ACK frame was expected but we got a good frame - treat as ACK timeout
				...
				case RTLS_DEMO_MSG_ANCHOR_CHECK: {
					non_user_payload_len = TAG_POLL_MSG_LEN;
					...
					inst->testAppState = TA_TXREPLY_WAIT_SEND; // send our response
					inst->canprintinfo = 0;

					memcpy(&inst->msg.destAddr[0], &srcAddr[0],
					ADDR_BYTE_SIZE_L); //remember who to send the reply to (set destination address)
				}
					break; //RTLS_DEMO_MSG_ANCHOR_CHECK

				case RTLS_DEMO_MSG_TAG_POLL: {
					inst->testAppState = TA_CALL_WAIT; // send our response
				}
					break; //RTLS_DEMO_MSG_ANCHOR_CALL
				...
				default: {
					...
					if (inst->doublebufferon == 0) {
						...
						dataseq[0] = 0x2;  //return cursor home
						writetoLCD(1, 0, dataseq);
						memcpy(&dataseq[0], (const uint8 *) " WRONG MSG      ", 16);
						writetoLCD(40, 1, dataseq);  //send some data
						memcpy(&dataseq[0], (const uint8 *) "RETURN RETURN   ", 16);
						writetoLCD(16, 1, dataseq);  //send some data
						*/
					}
					dwt_setrxaftertxdelay(0);
			...
			uplen = dw_event->rxLength
					- (non_user_payload_len + fctrladdr_len + FRAME_CRC);
		...
		default: {
			...
			dataseq[0] = 0x2;  //return cursor home
			writetoLCD(1, 0, dataseq);
			memcpy(&dataseq[0], (const uint8 *) " TA_CALL_WAIT   ", 16);
			writetoLCD(40, 1, dataseq);  //send some data
			memcpy(&dataseq[0], (const uint8 *) "RETURN RETURN   ", 16);
			writetoLCD(16, 1, dataseq);  //send some data
		...
		break; // end case TA_CALL_WAIT_DATA

	case TA_TXREPLY_WAIT_SEND: {
		...
		dataseq[0] = 0x2;  //return cursor home
		writetoLCD(1, 0, dataseq);
		memcpy(&dataseq[0], (const uint8 *) "    REPLY       ", 16);
		writetoLCD(40, 1, dataseq); //send some data
		memcpy(&dataseq[0], (const uint8 *) "    SEND END    ", 16);//calling send end
		writetoLCD(16, 1, dataseq); //send some data

		inst->testAppState = TA_TX_REPLY_WAIT_CONF;          // wait confirmation
		inst->previousState = TA_TXREPLY_WAIT_SEND;
		inst->done = INST_DONE_WAIT_FOR_NEXT_EVENT; //will use RX FWTO to time out (set below)

	}	//2015.03.05 JSH TA_TXREPLY_WAIT_SEND
		break;

	case TA_TX_REPLY_WAIT_CONF:
...

	{
		...
		if (inst->previousState == TA_TXREPLY_WAIT_SEND) {
		 ...
		 memcpy(&dataseq[0], (const uint8 *) "    CALLING     ", 16);
		 writetoLCD(40, 1, dataseq); //send some data
		 memcpy(&dataseq[0], (const uint8 *) "    AGAIN       ", 16);
		 writetoLCD(16, 1, dataseq); //send some data
		 inst->gotTO = 0;
		...
		break;
	}	//2015.03.05 JSH TA_TX_REPLY_WAIT_CONF
		break;
	...
	case TA_TXPOLL_WAIT_SEND: {
		...
		dataseq[0] = 0x2;  //return cursor home
		writetoLCD(1, 0, dataseq);
		memcpy(&dataseq[0], (const uint8 *) "      TAG       ", 16);
		writetoLCD(40, 1, dataseq);  //send some data
	...
	case TA_TXRESPONSE_WAIT_SEND: {
		...
		setupmacframedata(inst, ANCH_RESPONSE_MSG_LEN,
				...
				RTLS_DEMO_MSG_ANCH_RESP, !ACK_REQUESTED);
#endif
	...
	case TA_TXFINAL_WAIT_SEND: {
		...
		memcpy(&(inst->msg.messageData[PTXT]),
				(uint8 *) &inst->txu.tagPollTxTime, 5);

		// Write Response RX time field of Final message
		memcpy(&(inst->msg.messageData[RRXT]),
				(uint8 *) &inst->anchorRespRxTime, 5);
		...
		tagCalculatedFinalTxTime = tagCalculatedFinalTxTime
				+ inst->txantennaDelay;

		tagCalculatedFinalTxTime &= MASK_40BIT;

		// Write Calculated TX time field of Final message
		memcpy(&(inst->msg.messageData[FTXT]),
				(uint8 *) &tagCalculatedFinalTxTime, 5);
		...
		if (instancesendpacket(inst, DWT_START_TX_DELAYED)) {
			...
			///////////////////////////////////////REN 14.12.29/////////////////////////////////
			//if(inst->anchorListIndex == 3)
			//{
			inst->instToSleep = 0; //we'll sleep after this poll
			inst->anchorListIndex = 0; //start from the first anchor in the list after sleep finishes    -=1 -> 0  REN 2015.01.09
			//}
			////////////////////////////////////////////////////////////////////////////////////////////////
...
//NOTE - Auto ACK only works if frame filtering is enabled!
...

	{
        ...
        {
            inst->testAppState = TA_TXE_WAIT ;
            inst->nextState = TA_TXPOLL_WAIT_SEND ;    //123123
            break;
        }
        else if (inst->gotTO) //timeout
        {
	...
	case TA_RX_WAIT_DATA:
		...
		case SIG_RX_BLINK: {
			...
			if ((inst->mode == LISTENER) || (inst->mode == ANCHOR)) {
				...
				if (inst->tagToRangeWith < TAG_LIST_SIZE) {
					...
					if (memcmp(&inst->tagList[inst->tagToRangeWith][0],
							&(dw_event->msgu.rxblinkmsg.tagID[0]),
							BLINK_FRAME_SOURCE_ADDRESS) == 0) {
						inst->tagShortAdd = (dwt_getpartid() & 0xFF);
						inst->tagShortAdd = (inst->tagShortAdd << 8)
								+ dw_event->msgu.rxblinkmsg.tagID[0];
						...
						//set destination address
						memcpy(&inst->rng_initmsg.destAddr[0],
								&(dw_event->msgu.rxblinkmsg.tagID[0]),
								BLINK_FRAME_SOURCE_ADDRESS); //remember who to send the reply to
		...
		case DWT_SIG_RX_OKAY: {
			...
			if ((inst->ackexpected) && (inst->ackTO)) //ACK frame was expected but we got a good frame - treat as ACK timeout
				...
				case RTLS_DEMO_MSG_RNG_INIT: {
					...
					if (inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
						...
						inst->tagShortAdd = messageData[RES_R1]
								+ (messageData[RES_R2] << 8);
						instancesetreplydelay(
								messageData[RES_T1]
										+ (messageData[RES_T2] << 8), 0); //set the new timeouts/delay times for RX
...
#else
								memcpy(&inst->msg.destAddr[0], &srcAddr[0],
										ADDR_BYTE_SIZE_S); //set anchor address for the reply (set destination address)
				...
				case RTLS_DEMO_MSG_TAG_POLL: {
					...
					if (!inst->frameFilteringEnabled) {
						...
						DWT_FF_DATA_EN | DWT_FF_ACK_EN); //we are starting ranging - enable the filter....
						inst->frameFilteringEnabled = 1;
					}

					inst->tagPollRxTime = dw_event->timeStamp; //Poll's Rx time

					inst->delayedReplyTime = inst->tagPollRxTime
							+ inst->fixedReplyDelay; // time we should send the response
...
#else
					memcpy(&inst->msg.destAddr[0], &srcAddr[0],
							ADDR_BYTE_SIZE_S); //remember who to send the reply to (set destination address)
					tag_inst = inst;//2014.12.03 ljh
					//							memcpy(&tag_inst->msg.destAddr[0], &srcAddr[0], ADDR_BYTE_SIZE_L);// 2014.12.03 ljh
#endif
				}
				...
				case RTLS_DEMO_MSG_ANCH_RESP: {
					...
					inst->delayedReplyTime = inst->anchorRespRxTime
							+ inst->fixedReplyDelay; // time we should send the response
					inst->delayedReplyTime &= MASK_TXDTS;
				...
				case RTLS_DEMO_MSG_TAG_FINAL: {
					...
					aTxT = (inst->txu.anchorRespTxTime - inst->tagPollRxTime)
							& MASK_40BIT;
					...
					tRxT = (tagFinalRxTime - inst->txu.anchorRespTxTime)
							& MASK_40BIT;
					...
					if (inst->sendTOFR2Tag == SEND_TOF_REPORT) //////////REN 01.16   just note
					{
#if (DOUBLE_RX_BUFFER == 1)
						dwt_forcetrxoff(); //disable rx
#endif
			...
			if ((inst->instToSleep == 0)
					&& (inst->mode == LISTENER) /*|| (inst->mode == ANCHOR)*/) //update received data, and go back to receiving frames
					{
...
//dwt_setinterrupt(DWT_INT_TFRS | DWT_INT_RFCG | (DWT_INT_SFDT | DWT_INT_RFTO /*| DWT_INT_RXPTO*/), 1);
	dwt_setinterrupt(
			DWT_INT_TFRS | DWT_INT_RFCG
					| (DWT_INT_ARFE | DWT_INT_RFSL | DWT_INT_SFDT | DWT_INT_RPHE
							| DWT_INT_RFCE | DWT_INT_RFTO /*| DWT_INT_RXPTO*/),
			1);
...
#else
	int msgdatalen = RANGINGINIT_MSG_LEN + FRAME_CRTL_AND_ADDRESS_S
	+ FRAME_CRC;
#endif
...
#else
		msgdatalen = TAG_FINAL_MSG_LEN + FRAME_CRTL_AND_ADDRESS_S
		+ FRAME_CRC;
#endif
...
//preamble  = plen * (994 or 1018) depending on 16 or 64 PRF
	...
	if (datalength != 0) {
		...
		instance_data[instance].fwtoTimeB_sy = 16
				+ (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256)
				+ margin;

		instance_data[instance].rnginitW4Rdelay_sy = (int) (delayms * 1000
				/ 1.0256) - 16
				- (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256);
	} else {
		//set the frame wait timeout time - total time the frame takes in symbols
		instance_data[instance].fwtoTime_sy = 16
				+ (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256)
				+ margin;

		//this is the delay used for the delayed transmit (when sending the ranging init, response, and final messages)
		instance_data[instance].fixedReplyDelay = convertmicrosectodevicetimeu(
				delayms * 1e3);
		...
		instance_data[instance].fixedReplyDelay_sy = (int) (delayms * 1000
				/ 1.0256) - 16
				- (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256); //subtract 16 symbols, as receiver has a 16 symbol start up time
	}
