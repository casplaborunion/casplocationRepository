...
#define SOFTWARE_VER_STRING    "New TAG3" //
...
#if (DR_DISCOVERY == 0)
//Tag address list
uint64 tagAddressList[5] = { 0xDECA010000000001,         //  tag1
...
uint64 forwardingAddress[1] = { 0xDECA030000000001 };
...
const uint8 switchbuf[] = { 0, TA_SW1_3, TA_SW1_4, TA_SW1_5, TA_SW1_6, TA_SW1_7,
		TA_SW1_8 };
		...
		if (instancenewrange()) // have we got range details?
		{
			...
			writetoLCD(16, 1, dataseq1); //send some data
				...
				sprintf((char*) &dataseq[0], "t %4.2f %4d %4d %04d %d %d",
						range_result, instance_get_lcount(), instance_get_dly(),
						instance_get_respPSC(), instance_get_txl(),
						instance_get_rxl());
				//sprintf((char*)&dataseq[20], " %08x", dwt_read32bitoffsetreg(0xa, 1));
			} else {
				sprintf((char*) &dataseq[0], "a %4.2f %4d %4d %04d %d %d",
						range_result, instance_get_lcount(), instance_get_dly(),
						(dwt_read16bitoffsetreg(0x10, 2) >> 4),
