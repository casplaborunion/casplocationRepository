...
int testapprun_s(instance_data_t *inst, int message) {
	...
	case TA_INIT:
		...
		case TAG: {
			...
			uint8 eui64[8]; /////////////////////////////////////////////added by REN 01.17
			memcpy(eui64, &inst->payload.tagAddress, sizeof(uint64)); //////////////////////////////////added by//REN 01.17
			...
			memcpy(&inst->msg.sourceAddr[0], &inst->payload.tagAddress,
					ADDR_BYTE_SIZE_L); /////REN 01.17  &inst->payload.anchorAddress->&inst->tagAddress
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#if (ENABLE_AUTO_ACK == 1) //NOTE - Auto ACK only works if frame filtering is enabled!			//dwt_enableautoack(ACK_RESPONSE_TIME); //wait for ACK_RESPONSE_TIME symbols (e.g. 5) before replying with the ACK  //////REN. comment off 01.16
			dwt_setrxaftertxdelay(WAIT_FOR_RESPONSE_DLY); //set the RX after TX delay time///////////////////////added by REN 01.16
#endif
...
#else
			//////////////////////////////////////////////////////////////////////////////////////////////////
			// dwt_configuresleep(DWT_LOADUCODE|DWT_PRESRV_SLEEP|DWT_CONFIG|DWT_TANDV, DWT_WAKE_WK|DWT_WAKE_CS|DWT_SLP_EN); //configure the on wake parameters (upload the IC config settings)/////////////comment off by REN 01.16
		...
		case ANCHOR: {
#if (DR_DISCOVERY == 0)
			uint8 eui64[8];
			memcpy(eui64, &inst->payload.anchorAddress, sizeof(uint64)); /////////////////////payload->payload1  REN 01.17
			dwt_enableframefilter(DWT_FF_DATA_EN | DWT_FF_ACK_EN); //allow data, ack frames;
			inst->frameFilteringEnabled = 1;
			dwt_seteui(eui64);
#else
			...
			memcpy(&inst->msg.sourceAddr[0], &inst->payload.anchorAddress,
					ADDR_BYTE_SIZE_L); /////////////////////payload->payload1  REN 01.17
#else
					//set source address into the message structure
...
//#endif
	...
	case TA_TXE_WAIT: //either go to sleep or proceed to TX a message
		...
		///////////////////////////////////////REN 14.12.29//////REN comment off 01.17///////////////////////////
		...
		if (((inst->nextState == TA_TXPOLL_WAIT_SEND)
			...
			inst->testAppState = TA_RXE_WAIT; ///////////////////////////////////////////////////////////////////////////////REN 01.17   TA_SLEEP_DONE -> TA_RXE_WAIT
			...
			/*#if (DEEP_SLEEP == 1)
			 ...
			 #endif*/ ////////////////////////////////////////////////comment off by REN 01.17
	...
	case TA_TXPOLL_WAIT_SEND: {
		...
		//change back to original  REN 15.01.16

		//dwt_setrxtimeout((uint16)( (inst->fwtoTime_sy) * (2 + rand()%3) ));    //////////////////// REN 14.12.19
		//dwt_setrxtimeout((uint16)( (inst->fwtoTime_sy) * 4 ));               /////////////////////REN 14.12.29 when fixedreplydelay20ms, *4
...
//NOTE - Auto ACK only works if frame filtering is enabled!
		...
		// printf("TA_RXE_WAIT") ;
	{
		...
		char  dataseq[2][40];
					dataseq[0] = 0x2;  //return cursor home
					writetoLCD(1, 0, dataseq);
					memcpy(&dataseq[0], (const uint8 *) "    RXE_WAIT    ", 16);
					writetoLCD(40, 1, dataseq); //send some data
					memcpy(&dataseq[0], (const uint8 *) "    FINISH      ", 16);
					writetoLCD(16, 1, dataseq); //send some data
...
///////////REN 01.17/////////////////////////////////////////////////////////////
				case RTLS_DEMO_MSG_ANCHOR_CALL: {
					...
					//inst->tagPollRxTime = dw_event->timeStamp ; //Poll's Rx time      ///comment off including two rows below , REN 01.17
					...
					inst->testAppState = TA_TXPOLL_WAIT_SEND; // send our response      ////////////REN 01.17 TA_TXRESPONSE_WAIT_SEND -> TA_TXPOLL_WAIT_SEND
					inst->canprintinfo = 1;           ////////////REN 01.17 0->1
...
//////////////////////////////REN 01.17   for tag/////////////////////////////////////////////////////
//void instancesetaddresses_tag(instanceAddressConfig_t_tag *plconfig)
//    {
//   int instance = 0 ;

//    instance_data[instance].payload2 = *plconfig ;       // copy configurations

//   instancesetreporting(instance_data[instance].payload2.sendReport);
