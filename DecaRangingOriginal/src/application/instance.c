        ...
        case TA_INIT :
            ...
            switch (inst->mode)
            {
                case TAG:
                {
                    ...
                    memcpy(&inst->msg.sourceAddr[0], &inst->payload.tagAddress, ADDR_BYTE_SIZE_L);  /////REN 01.17  &inst->payload.anchorAddress->&inst->tagAddress
#endif
...
#if (DR_DISCOVERY == 0)
                    //set source address into the message structure
                	memcpy(&inst->msg.sourceAddr[0], &inst->payload.anchorAddress, ADDR_BYTE_SIZE_L);  /////////////////////payload->payload1  REN 01.17
#else
                    //set source address into the message structure
