...
case TA_INIT:
	...
	case TAG: {
		...
		inst->testAppState = TA_CALL_WAIT; /// TA_TXPOLL_WAIT_SEND -> TA_RXE_WAIT   REN 01.16
...
case TA_TXE_WAIT: //either go to sleep or proceed to TX a message
	...
	if (((inst->nextState == TA_TXPOLL_WAIT_SEND)
			|| (inst->nextState == TA_TXBLINK_WAIT_SEND)) && (inst->instToSleep) //go to sleep before sending the next poll
			) {
...
case TA_TXRANGINGINIT_WAIT_SEND: {
	...
	inst->rng_initmsg.messageData[RES_T1] = ((int) (inst->fixedReplyDelay_ms))
			& 0xFF;
	inst->rng_initmsg.messageData[RES_T2] = (((int) (inst->fixedReplyDelay_ms))
			>> 8) & 0xFF; //
...
#else
	setupmacframedata(inst, TAG_POLL_MSG_LEN,
			FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC, RTLS_DEMO_MSG_TAG_POLL,
			!ACK_REQUESTED);
#endif
...
#else
	setupmacframedata(inst, ANCH_RESPONSE_MSG_LEN,
			FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC, RTLS_DEMO_MSG_ANCH_RESP,
			!ACK_REQUESTED);
#endif
...
case TA_TXFINAL_WAIT_SEND: {
	...
	memcpy(&(inst->msg.messageData[PTXT]), (uint8 *) &inst->txu.tagPollTxTime,
			5);

	// Write Response RX time field of Final message
	memcpy(&(inst->msg.messageData[RRXT]), (uint8 *) &inst->anchorRespRxTime,
			5);
	...
	memcpy(&(inst->msg.messageData[FTXT]), (uint8 *) &tagCalculatedFinalTxTime,
			5);
...
#else
	setupmacframedata(inst, TAG_FINAL_MSG_LEN,
			FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC, RTLS_DEMO_MSG_TAG_FINAL,
			!ACK_REQUESTED);
#endif
...
#else
	setupmacframedata(inst, TOF_REPORT_MSG_LEN,
			FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC, RTLS_DEMO_MSG_ANCH_TOFR,
			((ENABLE_AUTO_ACK == 1) ? (ACK_REQUESTED) : (!ACK_REQUESTED)));
#endif
		...
		if ((inst->ackexpected) && (inst->ackTO)) //ACK frame was expected but we got a good frame - treat as ACK timeout
			...
			case RTLS_DEMO_MSG_RNG_INIT: {
				...
				if (inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
					...
					instancesetreplydelay(
							messageData[RES_T1] + (messageData[RES_T2] << 8),
							0); //set the new timeouts/delay times for RX
			...
			case RTLS_DEMO_MSG_TAG_FINAL: {
				...
				if (inst->sendTOFR2Tag == SEND_TOF_REPORT) {
					...
					dataseq[0] = 0x2;  //return cursor home
					writetoLCD(1, 0, dataseq);
					memcpy(&dataseq[0], (const uint8 *) "    FINAL       ", 16);
					writetoLCD(40, 1, dataseq); //send some data
					memcpy(&dataseq[0], (const uint8 *) "    ARRIVE      ", 16);
					writetoLCD(16, 1, dataseq); //send some data
...
case TA_CALL_WAIT_DATA:                                        // Wait RX data
	...
	//if we have received a DWT_SIG_RX_OKAY event - this means that the message is IEEE data type - need to check frame control to know which addressing mode is used
	{
		...
		if ((inst->ackexpected) && (inst->ackTO)) //ACK frame was expected but we got a good frame - treat as ACK timeout
			...
			if (fcode == RTLS_DEMO_MSG_ANCHOR_CALL) {
				non_user_payload_len = TAG_POLL_MSG_LEN;
...
dwt_setinterrupt(
		DWT_INT_TFRS | DWT_INT_RFCG
				| (DWT_INT_ARFE | DWT_INT_RFSL | DWT_INT_SFDT | DWT_INT_RPHE
						| DWT_INT_RFCE | DWT_INT_RFTO /*| DWT_INT_RXPTO*/), 1);
...
if (datalength != 0) {
	...
	instance_data[instance].fwtoTimeB_sy = 16
			+ (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256) + margin;

	instance_data[instance].rnginitW4Rdelay_sy = (int) (delayms * 1000 / 1.0256)
			- 16 - (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256);
} else {
	//set the frame wait timeout time - total time the frame takes in symbols
	...
	instance_data[instance].fixedReplyDelay_sy = (int) (delayms * 1000 / 1.0256)
			- 16 - (int) ((preamblelen + (msgdatalen / 1000.0)) / 1.0256); //subtract 16 symbols, as receiver has a 16 symbol start up time
}
